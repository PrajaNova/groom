...
export class EmailService {
  ...
  constructor(private fastify: FastifyInstance) {
    this.resend = new Resend(this.fastify.config.email.resendApiKey);
  }
  ...
  private generateJitsiUrl(meetingId: string): string {
    return `${this.fastify.config.app.url}/connect/${meetingId}`;
  }
  ...
  async sendBookingConfirmationEmail(params: {
    ...
    try {
      const { data, error } = await this.resend.emails.send({
        from: this.fastify.config.email.from,
        to: [to],
        subject: "Your Session is Confirmed - Groom",
        html: this.generateConfirmationEmailTemplate({
          name,
      ...
      if (error) {
        this.fastify.log.error({ error }, "Email send error");
  ...
  async sendBookingCancellationEmail(params: {
    ...
    try {
      const { data, error } = await this.resend.emails.send({
        ...
        html: this.generateCancellationEmailTemplate({
          name,
   ...
   */
  private generateConfirmationEmailTemplate(params: {
    name: string;
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f6fafb;">
   ...
   */
  private generateCancellationEmailTemplate(params: {
    name: string;
    originalTime: string;
  }) {
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f6fafb;">
  ...
  /**
   * Update/Reschedule email HTML template
   */
  private generateUpdateEmailTemplate(params: {
    name: string;
    newTime: string;
    meetingUrl: string;
    showLink: boolean;
